...
bool Swordii58::book(int start, int end) {
...
bool Swordii58::query(int start, int end, int l, int r, int idx) {
    if (r < start || end < l) {
        return false;
    }
    /* 如果该区间已被预订，则直接返回 */
...
void Swordii58::update(int start, int end, int l, int r, int idx) {
    if (r < start || end < l) {
        return;
    }
    if (start <= l && r <= end) {
...
bool Swordii58::book2(int start, int end) {

...
若一个 NFA 状态有一个标号非 $\varepsilon$ 的离开转换，则此状态为 **重要状态** (important state) 。

当满足以下条件， 两个 **NFA 状态集合** 可以被认为是一致的：

- 具有相同的重要状态。
- 要么都包含接受状态，要么都不包含接受状态。

NFA 的重要状态直接对应于正则表达式中存放了字母表中符号的位置，每个重要状态对应正则表达式的一个运算分量。

使用抽象语法树来表示扩展的正则表达式是非常有用的。该语法分析树的叶子结点对应于运算分量，内部结点表示运算符。

![抽象语法树](../pic/aReTree.png "抽象语法树")
...
> 如下图中含有以数字标记的重要节点：
>
> ![NFA2DFA-NFAend](../pic/NFA2DFA-NFAend.png "NFA2DFA-NFAend.png")

...
#pragma region DFS
void Solution515::dfs(TreeNode* root, std::vector<int>& ret, int depth) {
    if (depth + 1 > ret.size()) {
...
std::vector<int> Solution515::largestValues3(TreeNode* root) {
    if (!root) {
        return {};
    }
    std::vector<int> ret;
...
#pragma region 单栈
std::vector<int> Solution515::largestValues2(TreeNode* root) {
    std::vector<int> ans;

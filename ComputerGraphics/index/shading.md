...
![环境光](../pic/environmentLight.png)

对于环境光，无需记录其光源位置，只需记录其入射方向，因此可使用球体保存环境光信息。

![SpheromeMap](../pic/environmentLightBall.png)

然而将球体纹理展开为矩形时会发生形变。因此可将球体膨胀至立方体。

![CubeMap](../pic/envLightBox.png)

#### 法线贴图

在二维的纹理中定义深度信息，从而扰动着色点的法线，最终实现凹凸效果。

#### 位移贴图

位移贴图的输入与法线贴图一致，其直接移动顶点位置。

![位移贴图与法线贴图比较](../pic/differentMapping.png)

法线贴图是渲染时的欺骗，而位移贴图直接影响三角形顶点的计算，其最终效果与模型本身精度紧密相关。

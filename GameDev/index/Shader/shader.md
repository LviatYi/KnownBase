...
- 光栅化阶段 (Rasterizer Stage)
  - 光栅化阶段将利用顶点信息绘制屏幕上的像素，以渲染出最终的图像。
...
- 屏幕空间的阴影映射技术 （需要显卡支持 MRT）
  ...
  - 然后，根据光源的阴影映射纹理和摄像机的深度纹理来得到 **屏幕空间的阴影图** 。
    - 将摄像机的深度纹理和阴影映射纹理作为输入，在屏幕空间中对每个像素进行采样。
    - 根据采样到的深度值，将像素从摄像机的视角转换到光源的视角。
    - 将转换后的像素的深度值与阴影映射纹理进行比较，得到该像素是否在阴影中。
    - 将每个像素是否在阴影中的信息输出到屏幕空间的阴影图中，生成最终的阴影图。
